<#
.SYNOPSIS
Function to create a new Relativity ARM move job using Relativity's REST API.

.DESCRIPTION
This function constructs the required request, calls Relativity's REST API, and processes the response to create a new ARM move job.
...
.EXAMPLE
New-RelativityArmMoveJob -ArtifactID 1234567  -DatabaseServerID 2345671 -ResourcePoolID 3456712 -CacheLocationID 4567123 -FileRepositoryID 5671234

This example creates a new move job with the specified destination options.

.NOTES
Ensure you have connectivity and appropriate permissions in Relativity before running this function.
#>
function New-RelativityArmMoveJob
{
    ...
    Begin
    {
        Write-Verbose "Starting New-RelativityArmMoveJob"
    }
    Process
    {
        try
        {
            ...
            if ($PSCmdlet.ShouldProcess($ApiEndpoint))
            {
                ...
                Write-Verbose "Successfully created ARM move job with ID: $($Response.JobID)"
            }
    ...
    End
    {
        Write-Verbose "Completed New-RelativityArmMoveJob"
    }
}
